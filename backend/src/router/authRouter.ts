...
const AuthSchema = v.object({
  userName: v.pipe(
    v.string(),
    v.minLength(3,'username must be 3 or more characters long.')
  ),
  password: v.pipe(
    v.string(),
    v.minLength(6,'password must be 6 or more characters long.')
  )
})
...
authRouter.post(
  ...
  inputAuth,
  async (c: Context) => {
    const data = await (c.req as any).valid('json') as v.InferOutput<typeof AuthSchema>
    const exists = await prisma.user.findUnique({ where: { userName: data.userName } })
    if (exists) throw new HTTPException(400, { message: 'Username already exists' })
